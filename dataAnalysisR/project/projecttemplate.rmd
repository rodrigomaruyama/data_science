...
wdf.pca <- na.omit((wdf[, 1:11]))
wdf.quality <- factor(wdf[,12])
wdf.pca <- prcomp(wdf[,1:11], center = TRUE, scale. = TRUE)
g <- ggbiplot(wdf.pca, obs.scale = 1, var.scale = 1, groups = wdf.quality, ellipse = TRUE, circle = TRUE) + scale_color_discrete(name = '') + theme(legend.direction = 'vertical', legend.position = 'right')
